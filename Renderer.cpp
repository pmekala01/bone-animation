...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	//gui for the rotoations
	gui_1->addGroup("Bone Rotation");
	gui_1->addVariable("Rotate X1", m_bone_animation->rotations[0]);
	gui_1->addVariable("Rotate Y1", m_bone_animation->rotations[1]);
	gui_1->addVariable("Rotate Z1", m_bone_animation->rotations[2]);
	gui_1->addVariable("Rotate X2", m_bone_animation->rotations[3]);
	gui_1->addVariable("Rotate Y2", m_bone_animation->rotations[4]);
	gui_1->addVariable("Rotate Z2", m_bone_animation->rotations[5]);
	gui_1->addVariable("Rotate X3", m_bone_animation->rotations[6]);
	gui_1->addVariable("Rotate Y3", m_bone_animation->rotations[7]);
	gui_1->addVariable("Rotate Z3", m_bone_animation->rotations[8]);
	//add the stuff that will call the functions from bone animation file
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	root_bone_obj_mat = glm::translate(root_bone_obj_mat, m_bone_animation->root_positions[0]);
	...
	glm::mat4 purple_bone_obj_mat = glm::mat4(1.0f);
	glm::mat4 blue_bone_obj_mat = glm::mat4(1.0f);

	// initial bone placement
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, m_bone_animation->root_positions[3]);
	purple_bone_obj_mat = glm::translate(purple_bone_obj_mat, m_bone_animation->root_positions[2]);
	yellow_bone_obj_mat = glm::translate(yellow_bone_obj_mat, m_bone_animation->root_positions[1]);

	//rotate bones
	m_bone_animation->level = 1;
	blue_bone_obj_mat = m_bone_animation->blueRotate(blue_bone_obj_mat);
	m_bone_animation->level = 2;
	purple_bone_obj_mat = m_bone_animation->purpleRotate(purple_bone_obj_mat);
	blue_bone_obj_mat = m_bone_animation->blueRotate(blue_bone_obj_mat);
	m_bone_animation->level = 3;
	yellow_bone_obj_mat = m_bone_animation->yellowRotate(yellow_bone_obj_mat);
	purple_bone_obj_mat = m_bone_animation->purpleRotate(purple_bone_obj_mat);
	blue_bone_obj_mat = m_bone_animation->blueRotate(blue_bone_obj_mat);

	//scale the bones last
	blue_bone_obj_mat = glm::scale(blue_bone_obj_mat, m_bone_animation->scale_vector[3]);
	purple_bone_obj_mat = glm::scale(purple_bone_obj_mat, m_bone_animation->scale_vector[2]);
